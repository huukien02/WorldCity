...
import {
  useSellBuilding,
  SELL_REFUND_RATE,
} from "@/features/building/hooks/useSellBuilding";
...
export function TileInfoPanel() {
  ...
  const chunkId = selectedTile
    ? getChunkId(selectedTile.x, selectedTile.y)
    : "";
  const tileKey = selectedTile
    ? getTileKey(selectedTile.x, selectedTile.y)
    : "";
  ...
  const buildingConfig = tile?.buildingType
    ? BUILDING_CONFIG[tile.buildingType]
    : null;
  ...
  const srcTile = movingFrom
    ? (chunkData[srcChunkId]?.[srcTileKey] ?? null)
    : null;
  const srcBuildingConfig = srcTile?.buildingType
    ? BUILDING_CONFIG[srcTile.buildingType]
    : null;
  ...
  async function handleHarvest() {
    ...
    setLastEarned(null);
    try {
      const earned = await harvestTile(
        currentUser.uid,
        selectedTile.x,
        selectedTile.y,
      );
  ...
  async function handleSwap() {
    ...
    try {
      ...
      const msg =
        result === "swapped"
          ? "✓ Đã đổi chỗ công trình"
          : "✓ Đã di chuyển công trình";
      setError(msg);
  ...
  async function handleSell() {
    ...
    setError(null);
    try {
      const earned = await sellBuilding(
        currentUser.uid,
        selectedTile.x,
        selectedTile.y,
      );
  ...
  if (isSelectingDestination && srcBuildingConfig) {
    const dstBuildingConfig = tile?.buildingType
      ? BUILDING_CONFIG[tile.buildingType]
      : null;
    ...
    return (
      <div className="p-4 space-y-3">
        ...
        <div
          className={`p-3 rounded-lg border ${canConfirm ? "bg-slate-800 border-slate-600" : "bg-red-900/20 border-red-500/50"}`}
        >
          <p
            className={`text-xs font-medium mb-1 ${canConfirm ? "text-slate-400" : "text-red-400"}`}
          >
        ...
        {error && (
          <p
            className={`text-xs ${error.startsWith("✓") ? "text-emerald-400" : "text-red-400"}`}
          >
            {error}
          </p>
        )}
    ...
    return (
      <div className="p-4 space-y-4">
        <div>
          ...
          <p className="text-slate-400 text-xs">
            {BUILDING_EMOJI[tile!.buildingType!]} {buildingConfig.label} tại (
            {selectedTile.x}, {selectedTile.y})
          </p>
        </div>
        ...
        <div className="p-3 rounded-lg bg-slate-800 border border-slate-700 space-y-1.5 text-sm">
          <div className="flex justify-between text-slate-300">
            ...
            <span className="text-yellow-400">
              +{sellRefund.toLocaleString()} gold
            </span>
          </div>
          {pendingGold > 0 && (
            <div className="flex justify-between text-slate-300">
          ...
          <div className="border-t border-slate-700 pt-1.5 flex justify-between font-medium">
            ...
            <span className="text-emerald-400">
              +{sellTotal.toLocaleString()} gold
            </span>
          </div>
        </div>

        <p className="text-slate-500 text-xs">
          Công trình sẽ bị phá dỡ, đất vẫn thuộc về bạn.
        </p>
    ...
    <div className="p-4 space-y-4">
      ...
      {buildingConfig && (
        <div className="space-y-1">
          ...
          <p className="text-slate-400 text-xs">
            Level {tile?.buildingLevel ?? 1} · {buildingConfig.incomePerMinute}{" "}
            gold/phút
          </p>
        ...
        <>
          ...
          <div className="flex gap-2">
            <button
              onClick={() => {
                setError(null);
                setMovingFrom({ x: selectedTile.x, y: selectedTile.y });
              }}
              className="flex-1 py-2 rounded-lg border border-slate-600 text-slate-300 hover:text-white hover:border-slate-400 text-sm transition-colors"
            >
              Di chuyển
      ...
      {/* Nút xem điểm thành phố của người khác */}
      {!isEmpty && !isOwner && tile?.ownerId && (
        <button
          onClick={() => setShowScoreModal(true)}
          className="w-full py-2 px-4 rounded-lg border border-slate-700 hover:border-slate-500 text-slate-400 hover:text-white text-sm transition-colors flex items-center justify-center gap-2"
        >
          <span>🏙️</span>
          <span>Xem điểm thành phố</span>
        </button>
      )}

      {showScoreModal && tile?.ownerId && (
        <CityScoreModal
          uid={tile.ownerId}
          displayName={tile.ownerName ?? "Người chơi"}
