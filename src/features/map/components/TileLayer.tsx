...
function drawIsoDiamond(ctx: Context, sx: number, sy: number, shape: KonvaShape) {
  ctx.beginPath()
...
export const TileLayer = memo(function TileLayer({ currentUserId, minTile, maxTile }: TileLayerProps) {
  const { hoveredTile, selectedTile, movingFrom, setHoveredTile, setSelectedTile, chunkData } = useMapStore()
  ...
  const handleMouseMove = useCallback((e: KonvaEventObject<MouseEvent>) => {
    ...
    if (tileX !== undefined && tileY !== undefined) setHoveredTile({ x: tileX, y: tileY })
  }, [setHoveredTile])

  const handleMouseLeave = useCallback(() => setHoveredTile(null), [setHoveredTile])

  const handleClick = useCallback((e: KonvaEventObject<MouseEvent>) => {
    const tileX = e.target.getAttr('tileX') as number | undefined
    const tileY = e.target.getAttr('tileY') as number | undefined
    if (tileX !== undefined && tileY !== undefined) setSelectedTile({ x: tileX, y: tileY })
  }, [setSelectedTile])
  ...
  return (
    <Layer>
      {tiles.map(({ x, y, data }) => {
        ...
        const fill = getTileColor(data, isHovered, isSelected, isMovingSource, currentUserId)
        const stroke = isSelected ? '#fff' : isMovingSource ? '#93c5fd' : isHovered ? '#fde68a' : '#0f1117'
...
const IsoTile = memo(function IsoTile({
  screenX, screenY, tileX, tileY, fill, stroke, onMouseMove, onMouseLeave, onClick,
}: IsoTileProps) {
  return (
    <Shape
      tileX={tileX}
