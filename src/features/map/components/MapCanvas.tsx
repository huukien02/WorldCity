...
import { toIso, getVisibleChunks, getVisibleTileRange } from '../utils/isoCoords'
...
export function MapCanvas({ currentUserId }: MapCanvasProps) {
  ...
  const visibleChunks = getVisibleChunks(stagePos.x, stagePos.y, stageScale, size.w, size.h)
  useChunkLoader(visibleChunks)

  const { minTile, maxTile } = getVisibleTileRange(stagePos.x, stagePos.y, stageScale, size.w, size.h)
  ...
  return (
    <div ref={containerRef} className="w-full h-full overflow-hidden bg-[#0a0f1a] cursor-grab active:cursor-grabbing">
      <Stage
        width={size.w}
        height={size.h}
        x={stagePos.x}
